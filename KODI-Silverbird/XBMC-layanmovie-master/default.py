...
addon = xbmcaddon.Addon(id='plugin.video.silverbirdtv')
...
if mode == 'main':
    ...
    if languageFilter == 'false':
      for r in re.finditer('\<span class\=\"field\-content\"\>\<a href\=\"([^\"]+)\"\>([^\<]+)\<\/a\>\<\/span\>',
                         response_data, re.DOTALL):
        url,title = r.groups()
        addDirectory('plugin://plugin.video.silverbirdtv.com/live' + url,title)
        addDirectory('plugin://plugin.video.layanmovie/videos' + url,title + addon.getLocalizedString(30002))
#    else:
...
#      title = re.sub('\).*', ')', title)
#      addVideo('plugin://plugin.video.silverbirdtv/videos' + url, { 'title' : title , 'plot' : title },title, img=img)
#
#    match = re.compile('nextpostslink href="(.+?)"', re.DOTALL).findall(response_data)
#    for url in match:
#      addDirectory('plugin://plugin.video.layanmovie/videos=' + url,'>> '+addon.getLocalizedString(30006)+ ' >>')
...
#    title = plugin_queries['title']
    ...
    for r in re.finditer('<iframe src="([^\"]+)" (width=)',
                         response_data, re.DOTALL):
        url,width = r.groups()
        item = xbmcgui.ListItem(path='plugin://plugin.video.youtube/?mode=streamurl&url=' + url)
        log('play url: ' + 'plugin://plugin.video.youtube/?mode=streamurl&url=' + url)
